...
def do_http_test():
    resp = urllib2.urlopen('http://192.168.122.89:1337/')
    body = resp.read()
    print("response of /:%s" % body)
    if re.search(r'A brand new app', body) == None:
        return False
    try:
        resp = urllib2.urlopen('http://192.168.122.89:1337/invalid_url')
...
while True:
    ...
    if re.search(r'Sails started on port 1337', line) is not None:
        result =  do_http_test()
